  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      ...
      <div className="shrink-0 flex justify-between items-center pt-4 border-t border-[#e9ecef]">
        <div>
          <Button variant="ghost" onClick={() => navigate('/statement-formats')}>Cancel</Button>
        </div>
        <div className="flex gap-2.5">
          {step > STEP.SAMPLE && (
            <Button variant="secondary" onClick={prevStep} disabled={saving}>Back</Button>
          )}
          {!isLast && <Button onClick={nextStep} disabled={!canGoNext}>Next</Button>}
          {isLast && (
            <Button onClick={handleSave} loading={saving} disabled={!editor.validation.ok}>
