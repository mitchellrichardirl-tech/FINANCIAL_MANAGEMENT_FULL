/**
 * @file editor/fields/DateFormatField.jsx
 * Composite field managing the three `date_config` sub-fields:
 * `column`, `format`, and `dayfirst`.
 *
 * The format dropdown uses sentinels:
 *   __AUTO__   → stores `null` on the draft (let backend infer)
 *   __CUSTOM__ → reveals a free-text input; draft stores whatever's typed
 *   <preset>   → draft stores the strptime string directly
 */
...
export default function DateFormatField({ editor }) {
  ...
  // ── Derive dropdown state from the draft ───────────────────────────
  ...
  const handlePresetChange = (val) => {
    if (val === '__AUTO__') {
      ...
      // Switch to custom mode — seed with empty string only if we're
      // coming from auto or a preset; otherwise keep the existing string.
      if (isAuto || isKnownPreset) {
        updateDraft('date_config.format', '');
      }
    } else {
  ...
  const formatHelp = isAuto
    ...
    : isCustom
      ? undefined // help shown on the custom input below
      : undefined;
