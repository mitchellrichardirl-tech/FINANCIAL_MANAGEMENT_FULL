...
def low_confidence_receipt(mock_image):
    ...
    receipt = Receipt(
        ...
        page_number=1,
        original_image=mock_image,
        processed_images={"original": mock_image, "grayscale": mock_image},
...
class TestUpdateReceiptEndpoint:
    ...
    def test_update_receipt_vendor(
        self, mock_get_by_id, mock_update, client, sample_receipts
    ):
    ...
    def test_update_receipt_multiple_fields(
        ...
        response = client.put(
            "/api/receipts/1",
            data=json.dumps(
                {"vendor": "New Store", "amount": 150.00, "date": "2024-03-15"}
            ),
            content_type="application/json",
        )
        data = json.loads(response.data)
    ...
    def test_update_receipt_invalid_amount_string(
        self, mock_get_by_id, client, sample_receipts
    ):
    ...
    @patch.object(rm, "get_by_id")
    def test_update_receipt_invalid_confidence_float(
        self, mock_get_by_id, client, sample_receipts
    ):
    ...
    def test_update_receipt_unknown_fields_ignored(
        ...
        response = client.put(
            "/api/receipts/1",
            data=json.dumps({"vendor": "New Vendor", "unknown_field": "ignored"}),
            content_type="application/json",
        )
...
class TestDeleteReceiptEndpoint:
    ...
    def test_delete_receipt_success(
        self, mock_get_by_id, mock_delete, client, sample_receipts
    ):
    ...
    @patch.object(rm, "delete")
    @patch.object(rm, "get_by_id")
    def test_delete_receipt_with_file(
        self,
        mock_get_by_id,
        mock_delete,
        mock_unlink,
    ...
    @patch.object(rm, "delete")
    @patch.object(rm, "get_by_id")
    def test_delete_receipt_without_file_path(
...
class TestGetReceiptImageEndpoint:
    ...
    @patch.object(rm, "get_by_id")
    def test_get_receipt_image_success_jpg(
        self, mock_get_by_id, mock_exists, client, sample_receipts, tmp_path
    ):
    ...
    def test_get_receipt_image_file_not_exists(
        self, mock_get_by_id, mock_exists, client, sample_receipts
    ):
