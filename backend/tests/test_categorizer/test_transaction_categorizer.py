...
from src.utils.pandas_utils import SAME_PD

class TestTransactionCategorizerInitialization:
    """Test TransactionCategorizer initialization"""
    
    def test_init_default_parameters_no_db(self):
        categorizer = TransactionCategorizer(use_db=False)
        assert isinstance(categorizer.extractor, PartyExtractor)
        assert isinstance(categorizer.matcher, PartyMatcherRaw)
        assert categorizer.matcher.similarity_threshold == 80

    def test_init_default_uses_db_matcher(self, app_with_db):
    ...
    def test_init_custom_threshold(self):
        ...
        categorizer = TransactionCategorizer(similarity_threshold=90, use_db=False)
    ...
    def test_init_with_known_aliases(self):
        ...
        categorizer = TransactionCategorizer(known_aliases=aliases, use_db=False)
    ...
    def test_init_with_canonical_parties(self):
        ...
        categorizer = TransactionCategorizer(canonical_parties=canonical, use_db=False)
    ...
    def test_init_none_aliases_uses_empty_dict(self):
        ...
        categorizer = TransactionCategorizer(known_aliases=None, use_db=False)
    ...
    def test_init_none_canonical_uses_empty_dict(self):
        ...
        categorizer = TransactionCategorizer(canonical_parties=None, use_db=False)
    ...
    def test_init_doesnt_share_mutable_defaults(self):
        ...
        cat1 = TransactionCategorizer(use_db=False)
        cat2 = TransactionCategorizer(use_db=False)
    ...
    def test_init_invalid_threshold_raises_error(self):
        ...
        with pytest.raises(ValueError):
            TransactionCategorizer(similarity_threshold=101, use_db=False)
        
        with pytest.raises(ValueError):
            TransactionCategorizer(similarity_threshold=-1, use_db=False)
...
class TestCategorizeMethod:
    ...
    def test_categorize_creates_new_party(self):
        ...
        assert result[0]['party_id'] == 1
        assert "NEW" in categorizer.matcher.alias_mapping or \
               "NEW VENDOR" in categorizer.matcher.alias_mapping
...
class TestCategorizationWorkflow:
    ...
    def test_maintains_statistics(self):
        ...
        assert categorizer.matcher.new_aliases >= 1
        
        # Check discovered parties (AMAZON, NETFLIX)
        assert categorizer.matcher.new_parties == 2
...
class TestWithMockedDependencies:
    ...
    def test_categorize_calls_extractor(self, mock_extractor_class):
        """Test that categorize calls extractor methods"""

        mock_extractor = Mock()
        mock_extractor.clean_batch.return_value = pd.Series(["CLEANED"])
        mock_extractor.extract_party_names_batch.return_value = pd.Series(["EXTRACTED"])
        mock_extractor_class.return_value = mock_extractor
        
        categorizer = TransactionCategorizer(use_db=False)
        categorizer.categorize(["TEST"])

        mock_extractor.clean_batch.assert_called_once_with(
            SAME_PD(pd.Series(["TEST"]))
        )
        mock_extractor.extract_party_names_batch.assert_called_once_with(
            SAME_PD(pd.Series(["CLEANED"]))
        )

    @patch('src.categorizer.transaction_categorizer.PartyExtractor')
    def test_categorize_calls_matcher(self, mock_extractor_class):
        ...
        mock_matcher = Mock()
        mock_matcher.find_matches_batch.return_value = pd.DataFrame.from_dict(
            {
                "cleaned_description": ["MATCHED"],
                "party_id": [1],
                "confidence": [100]
            }
        )
        mock_matcher.get_new_counts.return_value = (1, 1)

        categorizer = TransactionCategorizer()
        categorizer.matcher = mock_matcher
        categorizer.categorize(["TEST"])
        
        categorizer.matcher.find_matches_batch.assert_called_once_with(
            SAME_PD(pd.Series(["EXTRACTED"]))
        )
