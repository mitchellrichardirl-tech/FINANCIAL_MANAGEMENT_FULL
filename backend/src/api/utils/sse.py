...
class SSEEventBuilder:
    ...
    @classmethod
    def fatal_error(cls, error_message: str, details: Optional[str] = None, **extra) -> str:
        """Create a fatal error event (no file_index: applies to the whole stream)."""
        data = {
            "status": "error",
            "fatal": True,
            "error": error_message,
    ...
    def heartbeat() -> str:
        """
        SSE comment line. The SSE spec says clients must ignore lines starting
        with ':', so this keeps proxies and the connection alive without the UI
        seeing an event.
        """
        return ": heartbeat\n\n"
...
def create_sse_response(
   ...
   headers: Optional[Dict[str, str]] = None
    ...
    logger.debug("Creating SSE response stream")
...
def create_error_sse_response(
   ...
   details: Optional[str] = None
    ...
    logger.warning(f"Creating error SSE response: {error_message}")
...
class BatchProcessor:
    ...
    def __init__(self, items: List[T], max_workers: int = 4):
        ...
        logger.debug(f"Initialized BatchProcessor: {self.total} items")
    ...
    def process_sequential(
        ...
        logger.info(f"Starting sequential processing: {self.total} items")
        ...
        for index, item in enumerate(self.items):
            ...
            try:
                ...
                logger.debug(f"Item {index} succeeded: {identifier}")
                ...
                logger.warning(f"Item {index} failed ({identifier}): {e}")
        ...
        logger.info(
            f"Sequential processing complete: {self.processed_count}/{self.total} "
