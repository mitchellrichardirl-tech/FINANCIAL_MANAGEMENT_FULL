...
class OCRLLMExtractor(ReceiptExtractorBase):
    """PaddleOCR + LLM. Stops after the first variant that yields good text."""

    PREFERRED = ("paddle_ready", "correct_skew", "denoise")

    def __init__(self, ocr: Optional[PaddleEngine] = None,
                 fields: Optional[LLMFieldExtractor] = None):
        super().__init__(ocr or PaddleEngine(use_gpu=True),
                         fields or LLMFieldExtractor())
